...
export default function HomePage({ videos, allVideos = [], loading, settings, category, query }) {
  ...
  const showHero = settings.ui.showHomeHero !== false;
  const hasFilters = (categoriesEnabled && category !== 'All') || query.trim();
  ...
  return (
    <main className="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-6">
      ...
      {settings.ads.enabled && settings.ads.topBannerEnabled !== false && (
        <div className={showHero ? 'my-5' : 'mb-5'}>
          <AdSlot html={settings.ads.topBannerHtml} label="Top advertisement" enabled />
        </div>
      )}

      <div className={`${showHero ? 'mt-7' : 'mt-2'} mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between`}>
        <div>
